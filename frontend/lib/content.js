...
export function getAllArticles(){
  return fs.readdirSync(dir).filter(f=>f.endsWith('.mdx')).map(file=>{
    ...
    return { slug:file.replace(/\.mdx$/,''), content, readingTime: Math.max(4, Math.round(content.split(/\s+/).length/180)), ...data }
...
export function articleJsonLd(article){return { '@context':'https://schema.org', '@type':'Article', headline:article.title, description:article.description, datePublished:article.date, dateModified:article.date, image:absolute(article.coverImage), author:{'@type':'Organization', name:'Редакция «На высоте»}, publisher:{'@type':'Organization', name:'На высоте', logo:{'@type':'ImageObject', url:absolute('/favicon.svg')}}, mainEntityOfPage:absolute(`/articles/${article.slug}`) }}
